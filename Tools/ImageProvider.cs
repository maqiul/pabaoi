     ...
     The image is then queued for the next grab.  */
        ...
        public uint GetDevice(string cameraId)
        {
            try
            {
                ...
                {
                    if (device.UserDefinedName.Contains(cameraId))
                    {
                        return device.Index;
                    }
                }
            ...
            public NODE_HANDLE GetNodeFromDevice(string name)
            {
                if (m_open && !m_removed)
                {
                    NODEMAP_HANDLE hNodemap = PylonC.NET.Pylon.DeviceGetNodeMap(m_hDevice);
            ...
            protected void Open(PYLON_DEVICE_HANDLE device)
            {
                try
                {
                    ...
                    if (PylonC.NET.Pylon.DeviceFeatureIsAvailable(m_hDevice, "EnumEntry_TriggerSelector_FrameStart"))
                    {
                        ...
                        PylonC.NET.Pylon.DeviceFeatureFromString(m_hDevice, "TriggerMode", "On");
                    }
            ...
            protected void Grab()
            {
                ...
                try
                {
                    ...
                    while (m_grabThreadRun) /* Is set to false when stopping to end the grab thread. */
                    {
                        ...
                        if (!PylonC.NET.Pylon.WaitObjectWait(m_hWait, 15000))
                        {
                            lock (m_lockObject)
                            {
            ...
            protected void CleanUpGrab()
            {
                ...
                /* ... The m_buffers can now be retrieved from the stream grabber. */
                {
                    ...
                    {
                        PylonGrabResult_t grabResult;  /* Stores the result of a grab operation. */
                        isReady = PylonC.NET.Pylon.StreamGrabberRetrieveResult(m_hGrabber, out grabResult);
